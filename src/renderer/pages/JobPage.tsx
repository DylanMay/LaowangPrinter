  ...
  return (
    <main className="flex flex-1 items-center justify-center px-6">
      <div className="w-[520px] max-w-full rounded-3xl border border-line bg-surface p-8">
        ...
        <p className="mt-2 text-[13px] leading-relaxed text-muted">{COPY.jobReady}</p>
        <ul className="mt-5 space-y-2 text-[14px]">
          <li>{connected ? '✓' : '○'} {COPY.deviceConnected}</li>
          <li>{imported ? '✓' : '○'} {COPY.importReady}</li>
          <li>{inBounds ? '✓' : '○'} {COPY.patternInBounds}</li>
          {gcode ? <li>✓ {COPY.timeLabel} {formatDuration(gcode.estimatedTime)}</li> : null}
          {workArea && placement ? (
            <li>
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <button
            type="button"
            onClick={() => showNotice(COPY.jobReady)}
            className="h-10 rounded-xl bg-ink px-4 text-sm font-semibold text-white"
          >
            {workMode === 'dry' ? COPY.startDryRun : COPY.startEngrave}
          </button>
        </div>
      </div>
    </main>
