...
- 工作方式如果还是「空载测试」，激光被故意关掉。先点「正常雕刻」选中它，再勾选确认项，点「开始雕刻」。
- 拔掉 USB 再插上，重新点「连接设备」。软件会把最小功率清零并打开激光模式；不重新连接的话，以前的固件设置还在。
- 若几秒后变成「雕刻机处于异常状态」，点顶部状态或「设置 → 调试信息」，把整页内容复制下来。常见原因是限位误报；新版本会在星光一类小床上关掉硬限位。调试页可以「解除异常」。
- 星光4N 激光头侧面如果有旋钮，转到打开。材料放到焦点上（按说明书，大约一指厚度）。
- 效果改成「深度」再试一次。点动、回原点、空载都不会开激光。要单独试光，到「设置 → 机器控制」打开激光头开关（需确认，功率较低）。
