...
连接成功后，如果 `$31` 大于 0，写入 `$31=0`；如果 `$32` 未打开，尝试 `$32=1`。激光模式下 S 低于 `$31` 时 PWM 为 0，表现就是能动但完全不出光。旧 GRBL-M3 没有 `$32` 时忽略错误，仍用 `M3` 开光。
...
所有 `M3`/`M4` 必须转为 `M5`，切削行上的 `S` 必须变为 `S0`。  
...
只使用：`G21` `G90` `G0` `G1` `M3` `M5` `S` `F`。每条切削 `G1` 都带 `S`，避免激光模式只在 `M3` 上设功率却不出光。
...
工作区最长边 ≤ 60 mm（星光4N 一类小床二极管）时，在材料预设上放慢进给、提高功率百分比，再换算 `S`。300 × 200 一类大床仍用原预设。

空载测试时不得发出有效的 `M3`/`M4` 开光，切削行上的 `S` 也必须变成 `S0`。
